...
with open('data/f5/driving_log.csv') as csvfile:
    reader = csv.reader(csvfile)
    for line in reader:
        line[0]='data/f5/'+line[0][20:]
        line[1]='data/f5/'+line[1][20:]
        line[2]='data/f5/'+line[2][20:]
        lines6.append(line)
lines6 = np.delete(lines6, 0, 0)


alllines=np.concatenate((lines1,lines2,lines3,lines4,lines5))
...
adamandeve = optimizers.Adam(lr=0.0001)
...
for line in lines6:
    ...
    inormal = prepare_image(image)
    inputi.append(inormal)
    m = float(line[3])
    expected.append(m)

x_test = np.array(inputi)
y_test = np.array(expected)
accuracy = model.evaluate(x_test,y_test)
# testing the model using a separate driving data

print("Scalar test loss is:",accuracy)
